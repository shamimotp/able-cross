        ...
        db.get('SELECT * from Dreams where id= "' + intercom.admin.id + '"', function (err, row) {
            if (row) {
              ...
              if(addonId=== undefined ||addonId === 'SELECTADDON' || curAddonId === addonId) {
                return CreateSubscription.process(db, intercom, res, intercom.current_canvas.stored_data.addons,currentAddons,intercom.current_canvas.stored_data.oldInputs);
              }
              let cbUser = row;
                chargebee.configure({
                    site: cbUser.sitename,
